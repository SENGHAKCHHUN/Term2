...
def draw(event):
    ...
    canvas.create_oval(x1, y1, x2, y2, fill= color_draw, outline=None)
def eraser():
    canvas.delete('all')
def chooseColor():
    global color_draw
...
frame = tk.Frame(win, width=1000, height=600, background='green' )
frame.pack()
draw_size = 5
color_draw = 'green'
...
btn = tk.Button(frame, text='Errase', height=5, width=10, command=eraser)
btn.pack()
btn_color = tk.Button(frame, text='choose color', command= chooseColor) 
btn_color.place(x= 10, y = 10)
...
HEIGHT = 400
WIDTH = 600
FALL_SPEED = 1  # Adjust this value to control the speed of falling
def apply_gravity():
    canvas.move(rectangle, 0, FALL_SPEED)  # Move the rectangle down
    if canvas.coords(rectangle)[3] < HEIGHT:  # Check if the rectangle is still in the canvas
        canvas.after(10, apply_gravity)  # Schedule the next iteration of gravity
root = tk.Tk()
canvas = tk.Canvas(root, width=WIDTH, height=HEIGHT)
canvas.pack()
rectangle = canvas.create_rectangle(50, 50, 100, 100, fill="blue")
apply_gravity()  # Start applying gravity
root.mainloop()