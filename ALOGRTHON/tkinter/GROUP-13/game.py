...
def scroll_background():
    canvas.move(background, -1, 0)  # Move the background image by (-1, 0) pixels
    x1, y1, x2, y2 = canvas.bbox(background)
    if x2 < canvas.winfo_width():
        canvas.move(background, canvas.winfo_width(), 0)  # Reset the background position if it goes off-screen
    canvas.after(10, scroll_background)  # Repeat the scrolling every 10 milliseconds
